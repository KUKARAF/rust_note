...
- [ ] **Excalidraw follow-ups** (full in-app editing shipped 2026-08-07: bare
  `.excalidraw` files, embedded editor via a lazy React island, server-side
  one-time wrapper migration). Remaining:
  - Shared/guest page (`/shared/{token}`) doesn't render drawings as SVG yet
    — it shows raw JSON text.
  - Consolidate the read-only `ExcalidrawView` onto the main
    `@excalidraw/excalidraw` package's `exportToSvg` instead of the
    separately-pinned `@excalidraw/utils` (`0.1.3-test32` — upstream only
    publishes test-tagged builds of the standalone utils package).
  - Concurrent-edit story is soft-lock + last-write-wins (a stale-CRDT
    "Repair drawing" banner exists as a backstop) — no real merge for
    drawings, by design; revisit only if multi-user editing of the same
    drawing becomes common.
  - `crates/mobile/tauri.conf.json` CSP is `null`; once tightened (see the
    CSP item above), it must allow the Excalidraw editor's own asset
    fetches from `/excalidraw-assets/` (same-origin, so likely already
    covered by a same-origin default).
